...
def main():
    ...
    for i in range(1,GEMNUM+1):
        ...
        if gemImage.get_size() != (GEMSIZE, GEMSIZE):
            gemImage = pygame.transform.smoothscale(gemImage, (GEMSIZE,GEMSIZE))
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE ):
                ...
                if event.pos == (lastMouseDownX, lastMouseDownY):
                    ...
                    #This is for dragging the mouse
...
def drawMovingGem(gem, progress):
    #Draw a gem sliding in the direction that its 'direction' key says.
    moveX = 0
    moveY = 0
...
def canMakeMove(board):
    ...
    #The patterns in oneOffPatterns represent the gems that are configured
...
def checkForGemClick(pos):
    #Check to see if mouse click was on the board
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
...
def pullDownAllGems(board):
    # pulls down gems on the board to the bottom to fill any gaps
    for x in range(BOARDWIDTH):
        gemsInColumn = []
        for y in range(BOARDHEIGHT):
            if board[x][y] != EMPTY_SPACE:
                gemsInColumn.append(board[x][y])
        board[x] = ([EMPTY_SPACE] * (BOARDHEIGHT - len(gemsInColumn))) + gemsInColumn                                                                        ([EMPTY_SPACE] * (BOARDHEIGHT - len(gemsInColumn))) + gemsInColumn
    ...
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT-2,-1,-1):
            if boardCopy[x][y+1] == EMPTY_SPACE and boardCopy[x][y] != EMPTY_SPACE:
                #If the space below a non-empty space is empty, drop the gem
...
def getSwappingGems(board, firstXY, secondXY):
    #If the two gems are adjacent, then their
    #'direction' keys are set to the right direction value and swapped
